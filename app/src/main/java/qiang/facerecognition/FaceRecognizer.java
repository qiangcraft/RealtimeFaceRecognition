...
public class FaceRecognizer extends TFliteTemplate {
    ...
    public Pair<Integer,Float> recognizeImage(Bitmap bitmap) throws FileNotFoundException {

        return recognizePerson(getFaceEmbeddings(bitmap));
    }

    private Pair<Integer,Float> recognizePerson(float[] embeddings) throws FileNotFoundException {

        return FaceFeature.search(embeddings, DATA_FILE);
